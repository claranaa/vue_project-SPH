...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 因为search页面是需要多次请求数据的，在mounted中只能请求一次，不合理
      // 把这次请求封装为一个函数，当需要请求数据的时候再调用即可
      // 在发请求之前带给服务器参数【searchParams参数发生变化有数值然后带给服务器】
      getData() {
      ...
      // 删除关键字
      removeKeywords() {
        // 给服务器带的参数searchParams的keywords置空
    ...
    /* computed: {
      ...mapState({
        // 太长了，不建议这样写，很容易写错，可以在仓库中的getter计算属性中写
