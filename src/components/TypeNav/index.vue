...
export default {
  ...
  computed: {
      ...mapState({
          // 对象的写法：右侧需要的是一个函数，当使用这个计算属性的时候，右侧的函数会立即执行一次
          // 注入一个参数state，其实即为大仓库中的数据{包括了小仓库home,search的数据}
          categoryList: (state) =>{
  ...
  methods: {
      ...
      changeIndex: throttle(function(index) {
          ...
          // 非正常情况（用户操作很快）：本身全部的一级分类都应该触发鼠标进入事件，但是经过测试，只有部分被触发了
          // 就是由于用户的行为过快，导致浏览器反应不过来。如果当前的回调函数中有一些大量的业务，有可能出现卡顿现象
      ...
      // 进行路由跳转的方法
      goSearch(event) {
          // 最好的解决方案：编程式导航路由+事件委派
          // 利用事件的委派存在一些问题：1. 事件委派，是把全部的子节点[h3,dt,dl,em]的事件委派给父节点
